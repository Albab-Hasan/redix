...
- `sizeof(type)`: `sizeof(int)` / `sizeof(unsigned)` → 4, `sizeof(char)` → 1, `sizeof(long)` → 8, and any pointer type including `sizeof(int **)` → 8
- `sizeof` applied to an expression: `sizeof(x)`, `sizeof(*p)`, `sizeof(a[0])`, `sizeof(s.field)`, `sizeof(p->field)` and `sizeof(f())`, with or without the parentheses, so `sizeof x` and `sizeof *p` work too. An array variable gives its whole size and a struct variable gives its laid-out size, which makes `malloc(sizeof(*node))` allocate the right number of bytes. The operand is never evaluated, so `sizeof(i++)` leaves `i` alone. The result has type `int` rather than `size_t`, and a string literal is typed as a pointer, so `sizeof("hi")` gives 8 where C gives 3. An array size still has to be a literal or a macro, so `char b[sizeof(struct t)]` does not parse
